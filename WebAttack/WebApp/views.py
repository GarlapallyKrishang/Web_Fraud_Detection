...
def PredictAction(request):
    if request.method == 'POST':
        ...
        output = '<table border="1" align="center" width="100%" ><tr><th><font size="" color="white">Test Data</th>'
        output += '<th><font size="" color="white">Predicted Value</th></tr>'
        for i in range(len(predict)):
            status = "Normal"
            if predict[i] == 0:
                status = "Abnormal"
            output+='<tr><td><font size="" color="white">'+str(temp[i])+'</td>'
            output+='<td><font size="" color="white">'+status+'</td></tr>'
...
def RunExisting(request):
    if request.method == 'GET':
        ...
        output = '<table border="1" align="center" width="100%" ><tr><th><font size="" color="white">Algorithm Name</th>'
        output += '<th><font size="" color="white">Accuracy</th><th><font size="" color="white">Precision</th>'
        output += '<th><font size="" color="white">Recall</th><th><font size="" color="white">FScore</th></tr>'
        for i in range(len(algorithms)):
            output+='<tr><td><font size="" color="white">'+algorithms[i]+'</td>'
            output+='<td><font size="" color="white">'+str(accuracy[i])+'</td>'
            output+='<td><font size="" color="white">'+str(precision[i])+'</td>'
            output+='<td><font size="" color="white">'+str(recall[i])+'</td>'
            output+='<td><font size="" color="white">'+str(fscore[i])+'</td></tr>'
        output+="</table><br/><br/><br/><br/><br/><br/>"
        context= {'data':output}
        return render(request, 'UserScreen.html', context)    
...
def sendOTP(email, otp_value):
    ...
    with smtplib.SMTP_SSL('smtp.gmail.com', 465) as connection:
        email_address = 'vijukrish18@gmail.com'
        email_password = 'nvgs nchg vcqi sshf'
        connection.login(email_address, email_password)
        connection.sendmail(from_addr="vijukrish18@gmail.com", to_addrs=em, msg="Subject : Your OTP : "+otp_value)
